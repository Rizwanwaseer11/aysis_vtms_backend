 ...
 * Bulk Activity:
 * - Supervisor only
 * - Two sub categories: KUNDI_CLEARANCE and LOADING (vehicle load events)
 * - Tracks which Kundi point, vehicle serving count, and before/after proofs
 */
...
deletedAt: { type: Date, default: null },
    bulk: {
      subType: { type: String, enum: ["KUNDI_BEFORE","VEHICLE_LOAD","KUNDI_AFTER"], required: true },
      kundiPointId: { type: mongoose.Schema.Types.ObjectId, ref: "KundiPoint", required: true },
      kundiPointName: { type: String, default: "" },
      vehicleVisitCount: { type: Number, default: 1 }
    }
  },
  { timestamps: true }
);
...
Schema.index({ "bulk.kundiPointId": 1, createdAt: -1 });
Schema.index({ "bulk.subType": 1, createdAt: -1 });

module.exports = mongoose.model("BulkActivity", Schema);
