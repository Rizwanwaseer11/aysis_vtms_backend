...
async function startShift(req, res, next) {
  try {
    ...
    if (user.operationType !== "FLAP") return fail(res, "User operation mismatch", null, 400);
...
if (!body.mtNumber) return fail(res, "mtNumber is required");
    ...
    const doc = await Model.create({
      ...
      flap: { mtNumber: String(body.mtNumber).trim(), mtVisitCount: Number(body.mtVisitCount || 1) }
    });
