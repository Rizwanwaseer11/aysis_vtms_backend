...
const Model = require("../../models/activities/BulkActivity");

/**
 * BULK Operation
 * Supervisor only. Kundi before, vehicle load, kundi after.
 */

/**
 * POST /operations/bulk/shift/start
...
async function startShift(req, res, next) {
  try {
    ...
    if (user.operationType !== "BULK") return fail(res, "User operation mismatch", null, 400);
...
if (!body.kundiPointId) return fail(res, "kundiPointId is required"); if (!body.subType) return fail(res, "subType is required");

    const shift = await AttendanceShift.findById(body.attendanceId);
    if (!shift) return fail(res, "Attendance shift not found", null, 404);
    if (shift.operationType !== "BULK") return fail(res, "Invalid shift operationType", null, 400);

    const { invoiceNo, monthKey } = await nextInvoice("BULK", null);
    ...
    const doc = await Model.create({
      ...
      bulk: { subType: body.subType, kundiPointId: body.kundiPointId, kundiPointName: body.kundiPointName || "", vehicleVisitCount: Number(body.vehicleVisitCount || 1) }
    });

    await cache.del("dashboardKpis");
    await cache.del("bulkList");

    return ok(res, "bulk activity created", doc, null, 201);
  } catch (e) { next(e); }
}

/**
 * PATCH /operations/bulk/shift/:id/end
 */
...
/**
 * GET /operations/bulk/list (admin table)
 */
async function list(req, res, next) {
  try {
    ...
    const cacheKey = cache.key(["bulkList", JSON.stringify(filter), page, perPage]);
    const cached = await cache.get(cacheKey);
    if (cached) return ok(res, "bulk activities", cached.data, cached.meta);
    ...
    return ok(res, "bulk activities", data, meta);
  } catch (e) { next(e); }
}
