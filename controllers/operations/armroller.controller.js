...
const Model = require("../../models/activities/ArmRollerActivity");

/**
 * ARMROLLER Operation
 * Driver only. Works with 5 cubic bin numbers.
 */

/**
 * POST /operations/armroller/shift/start
...
async function startShift(req, res, next) {
  try {
    ...
    if (user.operationType !== "ARMROLLER") return fail(res, "User operation mismatch", null, 400);
...
/**
 * POST /operations/armroller/activity
 */
...
if (!body.binNumber) return fail(res, "binNumber is required");

    const shift = await AttendanceShift.findById(body.attendanceId);
    if (!shift) return fail(res, "Attendance shift not found", null, 404);
    if (shift.operationType !== "ARMROLLER") return fail(res, "Invalid shift operationType", null, 400);

    const { invoiceNo, monthKey } = await nextInvoice("ARMROLLER", null);
    ...
    const doc = await Model.create({
      ...
      arm: { binId: body.binId || null, binNumber: String(body.binNumber).trim() }
    });

    await cache.del("dashboardKpis");
    await cache.del("armrollerList");

    return ok(res, "armRoller activity created", doc, null, 201);
  } catch (e) { next(e); }
}

/**
 * PATCH /operations/armroller/shift/:id/end
 */
...
/**
 * GET /operations/armroller/list (admin table)
 */
async function list(req, res, next) {
  try {
    ...
    const cacheKey = cache.key(["armrollerList", JSON.stringify(filter), page, perPage]);
    const cached = await cache.get(cacheKey);
    if (cached) return ok(res, "armRoller activities", cached.data, cached.meta);
    ...
    return ok(res, "armRoller activities", data, meta);
  } catch (e) { next(e); }
}
