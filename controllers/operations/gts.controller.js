...
const Model = require("../../models/activities/GtsActivity");

/**
 * GTS Operation
 * Supervisor only. Records vehicles dumping at selected GTS point.
 */

/**
 * POST /operations/gts/shift/start
...
async function startShift(req, res, next) {
  try {
    ...
    if (user.operationType !== "GTS") return fail(res, "User operation mismatch", null, 400);
...
if (!body.gtsPointId) return fail(res, "gtsPointId is required"); if (!body.vehicleNumber) return fail(res, "vehicleNumber is required");
    ...
    if (shift.operationType !== "GTS") return fail(res, "Invalid shift operationType", null, 400);

    const { invoiceNo, monthKey } = await nextInvoice("GTS", null);
    ...
    const doc = await Model.create({
      ...
      gts: { gtsPointId: body.gtsPointId, gtsPointName: body.gtsPointName || "", visitCount: Number(body.visitCount || 1) }
    });

    await cache.del("dashboardKpis");
    await cache.del("gtsList");

    return ok(res, "gts activity created", doc, null, 201);
  } catch (e) { next(e); }
}

/**
 * PATCH /operations/gts/shift/:id/end
 */
...
/**
 * GET /operations/gts/list (admin table)
 */
async function list(req, res, next) {
  try {
    ...
    const cacheKey = cache.key(["gtsList", JSON.stringify(filter), page, perPage]);
    const cached = await cache.get(cacheKey);
    if (cached) return ok(res, "gts activities", cached.data, cached.meta);
    ...
    return ok(res, "gts activities", data, meta);
  } catch (e) { next(e); }
}
